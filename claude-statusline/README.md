...
`swap status` が「来歴は未記録」と言っているのにここには名前が出ることがあります。食い違いではなく、問いが違うためです。`swap` は「そのスロットへ戻せるか」を見るので退避ファイルが消えていれば来歴を無効にしますが、こちらは「いまどのアカウントか」なので、退避を手で整理しても表示は変えません（消してしまうと、この表示が防ごうとしている“気づけない状態”を自分で作ることになります）。
