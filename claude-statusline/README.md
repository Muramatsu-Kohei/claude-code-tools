...
```
claude | Opus5 hi | ctx 4% 41k | $0.32 +2/-0 | 5h 26% 05:50 | 7d 7% Sat 09:00
```
...
| `ctx 4% 41k` | コンテキスト使用率と**使用**トークン数（入力 + 出力） |
| `$0.32` | セッション累計コスト |
| `+2/-0` | セッション中の増減行数 |
...
**`ctx` だけは使用率と絶対量の両方で色が変わります**（150k で黄、250k で赤）。使用率だけで判断すると 1M コンテキストでは危険域が見えないためです。1M 窓の 15% は 150k トークンですが、`ctx 15%` という表示は「まだ余裕がある」に見えてしまいます。実際にはターン単価が跳ね上がる領域なので、比率と絶対量のどちらか厳しいほうで色を付けています。
...
```
demo | Opus5 hi | ctx 4% 41k | $0.32 +12/-3 | 5h 26% ... | 7d 7% ...
```
...
**表示より後に呼ぶのは意図的です。** 記録側で何が起きてもステータスラインの表示を巻き込まないためで、フックの失敗は握りつぶされます。記録が動いていないことに表示から気づく手段はないので、記録が要るなら記録先を直接確認してください。
