...
function main() {
  ...
  for (const f of files) {
    ...
    // 固まる余地は残るので、ここでも締める。account-guard には *.test.js がちょうど 5 本
    // あるので、全ファイルが timeout しても 5 × 120000ms = 10 分に収まるよう 2 分に取って
    // ある(3 分だと 15 分になり job 側の timeout-minutes: 15 と等しくなってしまい、checkout
    // や setup-node の分だけ確実に超えて、ランナー自身の集計(「失敗: …」)が出る前に GitHub に
    // 殺される)。CI 実測は 3 ツール + セットアップ込みの全体で約 2 分、最長ファイルでも 1 分
    // 未満で、2 分にはその実測に対しても 2 倍の余裕がある。
