...
function main() {
  ...
  for (const f of files) {
    ...
    // 固まる余地は残るので、ここでも締める。このディレクトリの *.test.js は 2 本なので、
    // 全部が timeout しても 4 分で、job 側の timeout-minutes: 15 には十分収まる(CI は
    // 1 job = 1 ツールで回すので、この計算はこのディレクトリの本数だけで閉じる)。値は
    // 他の 2 ツールと揃えてあり、テストを増やしても 5 本までは 10 分に収まる。実測は 1 分未満。
