...
// ただし気付いてから動ける時間は短く、5h枠が 90% に達してから枠切れまでは実測で
// 中央 11〜12 分・最短 4 分しかない。「まだ余裕がある」と言い切れる段階は無いので、
// 閾値は余裕の有無ではなく「ここを逃すと記録の機会が無い」側から選ぶ(段ごとの
// 根拠は下の DEFAULTS と各 THRESHOLD に置く)。
//
...
// 無視する習慣がつく方が高くつく。
const DEFAULTS = {
  // 週次 90% で残り 10pt ≒ 5h枠 0.8 本(team は 1 本 = 12.4pt)。
  team: { five: 92, fiveFinal: 97, week: 90, weekFinal: 97 },
  // 週次 92% で残り 8pt ≒ 5h枠 0.8 本(max・pro は 1 本 = 9.8pt)。
...
// ACCOUNT には credentials の subscriptionType がそのまま入る(= 外部由来の文字列)ため、
...
function buildMessage(stages, st, resets) {
  ...
  //
  // 5h枠に限っては、最終段(97% = 残り約3pt)が /wrap 1回分(5〜10pt)に足りないことが
  // ある。承知のうえで置いている。確実に間に合わせるには /wrap が収まる1段目(92% =
  // 残り約8pt)で畳ませるしかないが、それは数時間で戻る枠のために毎回作業を止める
  // ことを意味し、中断のコストの方が大きい。5h枠の最終段は「間に合えば拾える最後の
  // 一手」であって、記録を保証する段ではない。週次枠側は 1pt が 5h枠の 8〜10pt に
  // 相当するので、同じ 97% でも /wrap には桁違いに足りる(この非対称は枠の容量差)。
  if (anyFinal) {
