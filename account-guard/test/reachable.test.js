...
// 新規 FAIL が増えた)。この基準に変えた当初は無改変でも 11 件が FAIL していた: いずれも cmdSwap の
// 「退避されていません」案内(`swap <名前>` / `swap save` とだけ言い切る経路)が、提示した名前の
// 状態(失効・subscriptionType 欠け・同一プラン・来歴とのずれ)や現在のログインの状態
// (名前未決定・credentials 破損)を考慮していないために起きる、同じ型の指摘だった。
// これらは本番コード側を直して解消済みで、いまは無改変で FAIL 0 が正しい状態。
// ここで FAIL が出たら既知の残件ではなく退行なので、必ず原因を追うこと。
const fs = require('fs');
...
//
// 変異版を置くときは credentials.js と subcommands.js も同じディレクトリにコピーすること。
// swap.js は require で隣を読むので、swap.js だけ置くと起動直後に落ち、全ケースで
