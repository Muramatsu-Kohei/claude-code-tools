...
function main() {
  ...
  for (const f of files) {
    ...
    // 5 × 180000ms = 15 分に収まり、job 側の timeout-minutes: 20 に checkout と setup-node の
    // 分を残せる(ここを詰めすぎると、ランナー自身の集計(「失敗: …」)が出る前に GitHub に
    // 殺され、どのファイルで固まったのか分からなくなる)。CI は 1 job = 1 ツールで回すので、
    // この計算はこのディレクトリの本数だけで閉じる。
    // 実測の最長は fault.test.js の約 60 秒。2 分だった頃の根拠は「実測 1 分未満に対して 2 倍」
    // だったが、swap warmup が操作列に入って子プロセス(切り替えと ping)を起こすようになり、
    // 同じファイルが約 38 秒から伸びて余裕が 2 倍を切った。手元より遅いランナーで本物のハングと
    // 区別できない失敗を出さないため、実測に対して 3 倍を保つ側に取り直した。
