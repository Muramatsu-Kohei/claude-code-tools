...
}
{
  ...
  const TWO = [{ tree: 'C:/org-tree', allow: ['team'] }, { tree: 'D:/org-tree2', allow: ['team'] }];
  const home = sandbox('deny-cross-tree', { subscriptionType: 'pro', rules: TWO });
  const res = run(home, {
    hook_event_name: 'PreToolUse', cwd: 'D:/org-tree2/work', tool_name: 'Read',
    tool_input: { file_path: 'C:/org-tree/proj/secret.py' },
  });
  ...
  check('cwd 注記は cwd 側のツリーを指す', /D:[\\/]org-tree2 配下のままだと/.test(reason), reason);
}
{
