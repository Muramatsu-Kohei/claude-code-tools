...
| `projects/<dir>/memory/MEMORY.md` | ディレクトリ単位 | 組織ツリーに入れば自動で載る |
...
→ **次に組織ツリー配下のリポジトリで Claude Code を開いた時点で追跡が始まり、以後は個人
アカウントのセッション開始時にも組織ツリーの作業内容が提示されうる。** 仮定ではなく、あと 1 回で発生する。
...
`C:\org-tree\<repo>` は別キーになるので、**自動注入で組織ツリーの内容が載ることはなかった。**
...
- [ ] 両窓オープン（swap を挟んで `claude-window-ping.ps1` を 2 回）を実装する（§6.2、仕様は §6.3）
...
**守るべき一線は 1 つ: `C:\org-tree` に入るときは必ず組織アカウント。**
...
> **本数は実装で少し増えた（2026-08-11）。** credentials に identity フィールドが無い以上、
> 同一プランの別アカウント同士は原理的に判別できない。取り違えた上書きで唯一のバックアップが
> 消えるのを防ぐため、既存スロットを上書きする前に旧内容を `accounts/.replaced/` へ控える
> 実装にした（スロットごとに 2 本まで。上限を超えた分だけ落とし、落とす順は失効済みから）。
> 「失効しているから」で無条件に消す実装は取りやめた（2026-08-11 の再レビュー）。退けた直後の
> 控えがその場で消え、時計がずれた環境では失効していない控えまで巻き添えになるため。
> 平文の本数はアカウント数 + 控え分に
> なるが、置き場所は同じ `~/.claude` 配下なので保護レベルは変わらない。判断で守れないものを
> 構造（復旧可能性）で守る方を選んだ、というトレードオフ。
...
**残る相手は所属組織**（Team 座席は組織との関係なので、組織側の規程が別に効く）。
...
swap 側のコマンドとして実装し、中身は既存の `claude-window-ping.ps1` を、アカウントを
入れ替えながら 2 回呼ぶだけにする（`-Force` は付けない。理由は §6.3 の決定表）。
単一アカウント利用者の既定動作は一切変わらない。ping の
