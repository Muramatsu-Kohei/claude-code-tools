...
! 現在のディレクトリは別アカウント専用のツリーのため表示していない。許可されたアカウントに切り替えれば見られる。
  ...
  (C:/org-tree は account-guard 側では保護されていない — 解除するつもりなら両方から外す)
```

この照合は**表示だけ**で、伏せる/見せるの判定には一切影響しない。名指しの注記(`--project` や
cwd 基準)では、その対象に実際に効いているルールだけを説明の材料にする。

**次のときは何も言わない。** どれも「保護されていない」と誤って案内する向きの事故になるため:

- account-guard を入れていない
- 向こうの設定が読めない・JSON として壊れている — account-guard は壊れた設定を全拒否として
  扱うので、「保護されていない」は事実と逆になる
