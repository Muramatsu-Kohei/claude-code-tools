...
ランナーは起動時に `test/.tmp` を丸ごと消すが、**名前で絞った実行ではこの掃除をスキップする**
(別スイートを並列で流しているときに、走っている側のサンドボックスを消さないため)。前回の
残骸を一掃したいときは引数なしで全実行すること。
...
- 除外が起きたときは黙って消さない。全プロジェクトを横断する表示 — `today`(`--project`
  を付けないとき)/ `list --all` / `export --all` — では「別アカウント専用のツリーのため
  N 件のプロジェクトを表示していません」と一行添える
  (SessionStart の自動注入では出力を汚さないよう、この注記は出さない)
- `--project` で対象を絞ったときは件数の注記を出さない代わりに、そのプロジェクトが
  制限に当たって空になった場合に「別アカウント専用のツリーのため表示していない」と伝える。
  「記録が無い」のか「見せていないだけ」なのかを区別できるようにするため
- `config.json` 自体が壊れて読めないときは、どのツリーを伏せるべきか判断できないので
  全ての記録を伏せる(その旨を注記する)。設定を直せば元に戻る
- 既定は空配列。これまでと完全に同じ挙動になる

#### 姉妹ツール account-guard との関係(設定は別物)
...
| ツール | 守るもの | 設定 |
| --- | --- | --- |
| [account-guard](../account-guard/README.md) | そのツリーへの**操作**を遮断する(読み書きとも) | `~/.claude/account-guard/config.json` の `rules` |
| claude-worklog | そのツリーの**作業ログの表示**を伏せる(記録は残す) | `~/.claude/worklog/config.json` の `restrictedTrees` |

分けてあるのは守備範囲が違うため。「操作は許すが作業ログは伏せる」も、その逆も書ける。
worklog は account-guard が無くても動く(依存しない)。

**保護を掛けるときも外すときも両方に書く。** 片方だけ外した状態で worklog が記録を伏せると、
注記にどちらの設定が効いているかを添える:

```
! 現在のディレクトリは別アカウント専用のツリーのため表示していない。許可されたアカウントに切り替えれば見られる。
  この制限は worklog 側の設定によるもの: C:\Users\<name>\.claude\worklog\config.json の restrictedTrees
  (C:/org-tree は account-guard 側では今のアカウントに効いていない — 解除するつもりなら両方の設定を見直す)
```

言うのは「**今のアカウントに効いていない**」であって「保護ルールが無い」ではない。向こうに
同じツリーのルールがあっても `allow` に今のアカウントが入っていれば今この瞬間は素通しなので、
食い違いとして挙げる。「無い」と書くと、真に受けて向こうのルールごと消したときに、他の
アカウントにまだ効いている保護(= 操作の遮断)まで外れる。

この照合は**表示だけ**で、伏せる/見せるの判定には一切影響しない。名指しの注記(`--project`、
cwd 基準、`move` の「対象外」一覧)では、その対象に実際に効いているルールだけを説明の材料にする。

**次のときは何も言わない。** どれも「効いていない」と誤って案内する向きの事故になるため:

- account-guard を入れていない
- 向こうの設定が読めない・JSON として壊れている — account-guard は壊れた設定を全拒否として
  扱うので、「効いていない」は事実と逆になる
- 向こうの `rules` に書き損じ(`tree` が相対パス・空・文字列でない)が **1 件でも** ある —
  account-guard は設定全体を壊れているとみなして全操作を拒否する。壊れたルールだけ捨てて
  残りで判定すると、向こうが最も強く保護している最中に「外せ」と案内してしまう。
  `allow` に誰が入っていても向こうの倒れ方は同じなので、この検査だけは全ルールに掛ける
- **どちらかの** `tree` がドライブ文字から始まらない(`/org-tree`、Git Bash 表記の
  `/c/org-tree`)— account-guard 側では有効なルールとして働くが、こちらのパス解決とは
  別の場所を指すので、同じツリーかどうかを判定できない。自分の `restrictedTrees` にも
  同じ検査を掛ける(`path.isAbsolute` は Windows でも `/org-tree` を通してしまうため)。
  向こうのルールで検めるのは**今のアカウントを拒否しているものだけ** — `allow` に今の
  アカウントが入っているルールは何も遮っておらず、書き方が食い違っても結論は変わらない
  (`guard status` 側も同じ範囲で絞っている)

ドライブ文字を必須にしている都合上、**この照合が働くのは Windows だけ**。POSIX では
常に何も出ない(制限そのものは今までどおり効く)。両ツールとも Windows 前提のため
割り切っている。

逆方向は `guard status` が同じ食い違いを報告する。
