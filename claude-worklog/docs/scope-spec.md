...
実装はまだ着手していない。**この文書だけを読んで実装できる**ことを目標に書いてある。
...
| # | 変更 | 根拠 | 単独コミット |
| --- | --- | --- | --- |
| 1 | `transcript_path` をフックから受け取り保存・引き渡し | F1 | **する**(現行の潜在バグ) |
| 2 | `repoKey()` 導入、`projectKey()` に正規化を追加 | D1 / F5 | する |
| 3 | `isMultiTool()` / `deriveScope()` を追加 | D2 / D3 | する |
| 4 | 注入をスコープ対応に | D4 / F6 | する |
| 5 | `list --scope` / `handoff <名前>` / `today` の表示 | — | まとめて可 |
| 6 | 並列警告を前方一致に | F7 | まとめて可 |
| 7 | 表示名を `cwd` 由来に | F8 | まとめて可 |
| 8 | `scopeMode` / `scopeIndexMax` / `scopeIndexMaxAgeDays` を設定に追加 | — | まとめて可 |
| 9 | `/wrap` `/finish` に `--scope`、`/handoff` に位置引数。README 更新 | — | する |
| 10 | `move` サブコマンド | F10 | 後回し |
...
```
worklog move --from <部分一致> --to <部分一致|キー> [--session <id>] [--all] [--dry-run]
```
