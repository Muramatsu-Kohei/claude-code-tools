...
> **旧 `state.json` の移行について。** 移行（`state-<account>.json` へのリネーム）は、
> アカウントを判別できた通常実行のときだけ行います。`-Status` は状態を見るだけの操作なので
> ファイルを動かさず、アカウントが `unknown`（未ログイン、`/login` 中で認証情報を読めない等）
> の回も移行を見送ります。取り違えて `state-unknown.json` へ退避してしまうと、本来の
> アカウントの記録が失われ、次回「前回 ping なし」と判定して意図しない時刻に ping が飛ぶためです。
> 移行を見送った回は旧 `state.json` をそのまま読むので、判定・表示は正しく行われます。
...
> 張り直したのかが分からないため）。ログインし直して再実行してください。
