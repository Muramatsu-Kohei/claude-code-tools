...
| `ping.log` | 実行ログ（PING / STATE / WARN / ERROR / DRYRUN の追記）。毎時発生しうる `SKIP` とアカウント判別不能の `WARN` は画面表示のみでファイルには残さない |
...
3. アカウントが `unknown`（未ログイン、`/login` 中、別ユーザー資格での実行など）→ `WARN` を画面に出して終了（ファイルには残さない）。`-Force` でも送らない
...
- **`unknown` の回はそもそも ping を送らない。** 枠はアカウントごとに独立しているので、
  ...
  へ分かれてしまう。ログインし直せば解消するので見送るほうが安い。WARN は SKIP と同様に
  画面のみに残し、ログイン切れが続く間に毎時積もって PING/STATE の履歴を押し出さないようにする。
